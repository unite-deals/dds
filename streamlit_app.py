...
# Hide Streamlit default UI + GitHub
st.markdown("""
<style>
...
def vote(username, option):
    c.execute("SELECT voted FROM users WHERE username=%s", (username,))
    result = c.fetchone()

    if result and result[0] == 1:
        return False

    c.execute("INSERT INTO votes (option) VALUES (%s)", (option,))
    c.execute("UPDATE users SET voted=1 WHERE username=%s", (username,))
    return True
...
# -------------------- MAIN APP --------------------
    ...
    lal = percent(res.get("লাল", 0))
    gerua = percent(res.get("গেরুয়া", 0))
    sabuj = percent(res.get("সবুজ", 0))

    st.write(f"🔴 লাল: {lal:.2f}%")
    st.write(f"🟠 গেরুয়া: {gerua:.2f}%")
    st.write(f"🟢 সবুজ: {sabuj:.2f}%")
